...
2026-06-27 | Exception handling / thread entry points | Wrapped every remaining unguarded thread loop body in rs2v::Guard so a recoverable throw is non-fatal instead of std::terminate: NetworkThread::RunLoop (PumpNetwork + tick cb, untrusted traffic), EACServerEmulator::RunLoop (HandlePacket + cleanup, untrusted UDP), Timer once/repeating callbacks, GameServer detached auto-regen, ProtocolDecoder::WorkerLoop; added catch(...) to TelemetryManager::SamplingLoop. Logged 3 silent parser swallows (CommandHandlers::ParseFloat, MapManager lighting/ambient, ChatManager /votemap). +cross-thread Guard regression test. 26/26 green. | (this commit)
