...
2026-06-26 | Pre-auth gameplay gate (OPEN finding #1) | FIXED: the legacy string-tag dispatch (GameServer::Run) ran the state-changing gameplay handlers (ROLE_SELECT/SPAWN_REQUEST/COMMANDER_ABILITY/SQUAD_ACTION/VEHICLE_ACTION/WEAPON_FIRE + the GameMode action fallthrough) with NO auth/connection-state check, so any client that never logged in could drive gameplay pre-auth (this path is reachable by any non-UE3 datagram). Added a gate: every tag except CHAT_MESSAGE now requires conn->IsHandshakeComplete(). Real UE3 clients never use this legacy path (their gameplay rides the UE3 control channel via DecodeInboundActorBunch), so no impact; CHAT stays allowed (already length-capped). Validated react PASS + spawn PASS. | ec4b9bb
2026-06-26 | Weapon-fire attacker/victim guards (OPEN finding #4) | FIXED: HandleWeaponFire trusted an attacker-supplied victimId with NO attacker-state check - a dead/spectating/never-spawned client could claim kills on any id. Added guards: attacker must resolve to a live Player (else reject) and victim must resolve to a live Player (else reject the spoofed/garbage hit); also removes the former double GetPlayer() lookup. No range/LOS gate (damage falloff means there is no hard max range, so a distance gate would false-positive on long shots). FF still handled by DamageSystem. Validated react PASS + spawn PASS. | cc79926
2026-06-26 | Admin ban-duration parse guard (OPEN finding #5) | FIXED: AdminManager "ban" did int duration=std::stoi(args[1]) on operator text - a mistyped/non-numeric duration ("ban player forever") threw std::invalid_argument (or std::out_of_range on huge values) and crashed the whole server. Wrapped in try/catch(...) with a consumed-length check (rejects trailing junk like "10abc"), negative clamp, and a usage message back to the admin. Admin-only (not wire-reachable) but a real crash. Build green, react PASS + spawn PASS. | (this commit)
