...
MapConfig::MapConfig() {
    Logger::Info("MapConfig initialized");
}

MapConfig::~MapConfig() = default;

bool MapConfig::Initialize(const std::string& configPath) {
    Logger::Info("Initializing MapConfig from %s", configPath.c_str());
    m_configPath = configPath;
    if (!std::filesystem::exists(configPath)) {
        Logger::Warn("MapConfig file not found, creating default: %s", configPath.c_str());
        CreateDefaultConfig();
        return Save();
    }
    return Load();
...
std::vector<std::string> MapConfig::GetAvailableMaps() const {
    ...
    for (auto& kv : m_mapDefinitions) keys.push_back(kv.first);
    return keys;
}