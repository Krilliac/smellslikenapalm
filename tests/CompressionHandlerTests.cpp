...
//
// zlib is an OPTIONAL dependency. When rs2v_core is built with it
// (RS2V_HAS_ZLIB), the ZLIB path is exercised directly; otherwise the ZLIB
// cases skip (rather than fail) since the handler falls back to a stub. The
// NONE passthrough path is always available.
...
// Skip a ZLIB test when zlib was not compiled into rs2v_core.
#ifdef RS2V_HAS_ZLIB
#define RS2V_REQUIRE_ZLIB() ((void)0)
#else
#define RS2V_REQUIRE_ZLIB() do { SKIP_TEST() << "built without zlib (RS2V_HAS_ZLIB undefined)"; } while (0)
#endif
